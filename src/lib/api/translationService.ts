...
export class TranslationService {
  ...
  constructor(apiKey?: string, geminiApiKey?: string) {
    ...
    this.geminiApiKey = geminiApiKey || process.env.GEMINI_API_KEY || 'AIzaSyBAhXOXwQuk2DHDP7gMDa3EJpZPQIrYzlo';
   ...
   */
  private async getLLMVerdict(request: LLMVerdictRequest): Promise<LLMVerdictResponse> {
    try {
      console.log('🤖 [LLM VERDICT] Starting LLM review process...');
      console.log('📝 [LLM VERDICT] Original texts:', request.originalTexts);
      console.log('🔄 [LLM VERDICT] Google translations:', request.translations);
      console.log('🌐 [LLM VERDICT] Language pair:', `${request.sourceLanguage} → ${request.targetLanguage}`);
      
      const prompt = this.buildLLMPrompt(request);
      console.log('💭 [LLM VERDICT] Generated prompt length:', prompt.length, 'characters');
  ...
  private buildLLMPrompt(request: LLMVerdictRequest): string {
    const { originalTexts, translations, sourceLanguage, targetLanguage, detectedLanguage } = request;
...
${index + 1}. Original: "${text}"
   Translation: "${translations[index]}"`).join('\n')}
...
Provide only the JSON response, no additional text.`;
   ...
   */
  private parseLLMResponse(response: string, request: LLMVerdictRequest): LLMVerdictResponse {
    try {
      ...
      if (!parsed.correctedTranslations || !Array.isArray(parsed.correctedTranslations)) {
      ...
      if (parsed.correctedTranslations.length !== request.translations.length) {
        console.error('❌ [LLM VERDICT] Mismatch in number of translations:', 
      ...
      console.log('🔄 [LLM VERDICT] Corrected translations:', parsed.correctedTranslations);
      console.log('🌐 [LLM VERDICT] Language correction:', parsed.languageCorrection);
      ...
      console.log('🔄 [LLM VERDICT] Returning original translations as fallback');
  ...
  async translateText(
    ...
    try {
      ...
      if (options.sourceLanguage) {
        ...
        console.log('🔤 [TRANSLATE] Source language specified:', options.sourceLanguage);
      }
      ...
      if (data.data.translations && data.data.translations.length > 0) {
        ...
        if (options.useLLMVerdict) {
          console.log('🤖 [TRANSLATE] LLM verdict requested, preparing request...');
          ...
          if (llmVerdict.correctedTranslations[0] !== translation.translatedText) {
            console.log('🔄 [TRANSLATE] LLM improved the translation!');
            console.log('🔄 [TRANSLATE] Original:', translation.translatedText);
            console.log('🔄 [TRANSLATE] Improved:', llmVerdict.correctedTranslations[0]);
          ...
          if (llmVerdict.languageCorrection && llmVerdict.languageCorrection !== result.detectedSourceLanguage) {
            console.log('🌐 [TRANSLATE] LLM corrected language detection:');
            console.log('🌐 [TRANSLATE] Original detection:', result.detectedSourceLanguage);
            console.log('🌐 [TRANSLATE] LLM correction:', llmVerdict.languageCorrection);
  ...
  async translateTexts(
    ...
    try {
      ...
      console.log('📡 [BATCH TRANSLATE] Getting translations from Google Translate...');
      for (let i = 0; i < validTexts.length; i++) {
        const text = validTexts[i];
        console.log(`🔄 [BATCH TRANSLATE] Translating text ${i + 1}/${validTexts.length}: "${text}"`);
        ...
        if (data.data.translations && data.data.translations.length > 0) {
          const translation = data.data.translations[0];
          console.log(`✅ [BATCH TRANSLATE] Google translation ${i + 1}:`, translation.translatedText);
          ...
          console.error(`❌ [BATCH TRANSLATE] No translation received for text: "${text}"`);
      ...
      if (options.useLLMVerdict && googleTranslations.length > 0) {
        console.log('🤖 [BATCH TRANSLATE] LLM verdict requested for batch review...');
        ...
        console.log(`📊 [BATCH TRANSLATE] LLM improved ${improvementCount}/${googleTranslations.length} translations`);

        // Update detected language if LLM corrected it
        if (llmVerdict.languageCorrection) {
          console.log('🌐 [BATCH TRANSLATE] LLM language correction applied:', llmVerdict.languageCorrection);
      ...
      console.log('🎉 [BATCH TRANSLATE] Final batch translation results:', googleTranslations);
...
export const translateWithLLMVerdict = async (
  ...
  return getTranslationService().translateWithLLMVerdict(texts, targetLanguage, sourceLanguage);
};
...
export type { TranslationOptions, TranslationResult, Language, LLMVerdictRequest, LLMVerdictResponse };
