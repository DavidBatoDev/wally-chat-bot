    ...
    <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="space-y-6">
          <div className="bg-green-100 p-4 rounded">
            ...
            <p className="text-green-700">
              Successfully serialized {serializedData.totalTextBoxes} textboxes
              across {serializedData.pageDimensions.size} pages
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-gray-100 p-4 rounded">
              ...
              <div className="space-y-2">
                <div>
                  ...
                  >
                    {Array.from(serializedData.pageDimensions.keys()).map(
                      (pageNum) => (
            ...
            <div className="bg-gray-100 p-4 rounded">
              ...
              <div className="space-y-1 text-sm">
                <p>
                  <strong>Total TextBoxes:</strong>{" "}
                  {serializedData.totalTextBoxes}
                </p>
                <p>
                  <strong>Pages:</strong> {serializedData.pageDimensions.size}
                </p>
                <p>
                  <strong>Current Page TextBoxes:</strong>{" "}
                  {getPageTextBoxes().length}
                </p>
                <p>
                  <strong>Page Dimensions:</strong>{" "}
                  {serializedData.pageDimensions.get(selectedPage)?.width || 0}{" "}
                  ×{" "}
                  {serializedData.pageDimensions.get(selectedPage)?.height || 0}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="p-4">
              {getPageTextBoxes().length > 0 ? (
                <div className="space-y-3">
                  {getPageTextBoxes().map(
                    (textBox: TextField, index: number) => (
                      <div
                        key={textBox.id}
                      ...
                      >
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                          ...
                          <div>
                            <strong>Position:</strong> ({textBox.x.toFixed(1)},{" "}
                            {textBox.y.toFixed(1)})
                          </div>
                          <div>
                            <strong>Size:</strong> {textBox.width.toFixed(1)} ×{" "}
                            {textBox.height.toFixed(1)}
                          </div>
                          <div>
                            <strong>Font:</strong> {textBox.fontSize}px{" "}
                            {textBox.fontFamily}
                          </div>
                          <div>
                            <strong>Color:</strong>{" "}
                ...
                <p className="text-gray-500">
                  No textboxes found for this page and view.
                </p>
              )}
            </div>
          </div>
                  ...
                  (textBox: TextField, index: number) => (
                    ...
                    >
                      ...
                      <span className="w-16 text-xs">
                        ({textBox.x.toFixed(0)}, {textBox.y.toFixed(0)})
                      </span>
                    </div>
                  )
                )}
      ...
      <div className="mt-8 bg-blue-50 p-4 rounded">
        ...
        <div className="text-sm text-blue-700 space-y-2">
          <p>
            <strong>1. Import the serializer:</strong>
          </p>
          <code className="block bg-blue-100 p-2 rounded font-mono">
            import {(serializeOcrResponse, getTextBoxesForPage)} from
            "./utils/ocrResponseSerializer";
          </code>

          <p>
            <strong>2. Serialize OCR response:</strong>
          </p>
          <code className="block bg-blue-100 p-2 rounded font-mono">
            const {(textBoxesByPage, pageDimensions, totalTextBoxes)} =
          ...
          <p>
            <strong>3. Get textboxes for specific page:</strong>
          </p>
          <code className="block bg-blue-100 p-2 rounded font-mono">
            const pageTextBoxes = getTextBoxesForPage(textBoxesByPage, 1,
            "original");
          </code>

          <p>
            <strong>4. Create textboxes in frontend:</strong>
          </p>
          <code className="block bg-blue-100 p-2 rounded font-mono">
            {`pageTextBoxes.forEach(textBox => {
              handleAddTextBoxWithUndo(textBox.x, textBox.y, textBox.page, "original", "original", textBox);
            });`}
          </code>
        </div>
      </div>
