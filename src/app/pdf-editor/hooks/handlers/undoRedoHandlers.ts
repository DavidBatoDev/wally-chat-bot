...
import { TextField, ViewMode, Shape as ShapeType, Image as ImageType, DeletionRectangle } from "../../types/pdf-editor.types";
import {
  AddTextBoxCommand,
...
export function useHandleAddTextBoxWithUndo(
  addTextBox: (x: number, y: number, page: number, view: ViewMode, targetView?: ViewMode, initialProperties?: Partial<TextField>) => string,
...
export function useHandleDuplicateTextBoxWithUndo(
  duplicateTextBox: (originalId: string, currentView: ViewMode) => string | null,
  deleteTextBox: (id: string, view: ViewMode) => void,
  history: any,
  elementCollections: any
) {
  return useCallback(
    (originalId: string, currentView: ViewMode, page: number) => {
      ...
      const originalTextBox = allTextBoxes.find((tb: any) => tb.id === originalId);
...
export function useHandleUpdateTextBoxWithUndo(
  ...
  return useCallback(
    (
      ...
      const command = new UpdateTextBoxCommand(updateTextBox, id, before, after);
...
export function useUpdateTextBoxWithUndo(
  ...
  return useCallback(
    ...
    [getCurrentTextBoxState, handleUpdateTextBoxWithUndo, documentState, viewState, history]
  );
}
...
export function useUpdateOriginalTextBoxWithUndo(
  ...
  return useCallback(
    ...
    [getCurrentTextBoxState, handleUpdateTextBoxWithUndo, documentState, history]
  );
}
  ...
  return useCallback(
    (
      ...
      const addFunc = (shape: ShapeType) => {
        const id = addShape(type, x, y, width, height, page, view, targetView, x1, y1, x2, y2);
        shape.id = id;
        return id;
      };
...
export function useHandleUpdateShapeWithUndo(
  updateShape: any,
  history: any
) {
  return useCallback(
...
export function useUpdateShapeWithUndo(
  ...
  return useCallback(
    ...
    [getCurrentShapeState, handleUpdateShapeWithUndo, elementCollections, history]
  );
}
...
export function useHandleDeleteTextBoxWithUndo(
  ...
  return useCallback(
    (id: string, view: ViewMode) => {
      ...
      const textBoxView = elementCollections.originalTextBoxes.some((tb: any) => tb.id === id)
        ? "original"
      ...
      const deleteFunc = (id: string) => {
        console.log('[DeleteTextBox] Deleting textbox', id, 'from view', textBoxView);
        deleteTextBox(id, textBoxView);
      };
      
      const restoreFunc = (tb: TextField, view: ViewMode) => {
        console.log('[DeleteTextBox] Restoring textbox', tb.id, 'to view', view);
        restoreTextBox(tb, view);
      };

      const command = new DeleteTextBoxCommand(deleteFunc, restoreFunc, textBox, textBoxView as ViewMode);
      history.executeCommand(command);
    ...
    [deleteTextBox, restoreTextBox, history, elementCollections, editorState, selectedElementId, clearSelectionState]
  );
}
...
export function useHandleDeleteShapeWithUndo(
  ...
  return useCallback(
    (id: string, view: ViewMode) => {
      ...
      const shapeView = elementCollections.originalShapes.some((s: any) => s.id === id)
        ? "original"
      ...
      const command = new DeleteShapeCommand(deleteFunc, restoreFunc, shape, shapeView as ViewMode);
      history.executeCommand(command);
    ...
    [deleteShape, restoreShape, history, elementCollections, editorState, selectedElementId, clearSelectionState]
  );
}
  ...
  return useCallback(
    (
      ...
      const addFunc = (rect: DeletionRectangle) => {
        const id = addDeletionRectangle(x, y, width, height, page, view, background, opacity);
        rect.id = id;
        return id;
      };
      ...
      const command = new AddDeletionRectangleCommand(addFunc, deleteFunc, rectData);
      history.executeCommand(command);
...
export function useHandleDeleteDeletionRectangleWithUndo(
  ...
  return useCallback(
    (id: string, view: ViewMode) => {
      ...
      const rectView = elementCollections.originalDeletionRectangles.some((r: any) => r.id === id)
        ? "original"
        : elementCollections.finalLayoutDeletionRectangles.some((r: any) => r.id === id)
        ? "final-layout"
        : "translated";

      const deleteFunc = (id: string) => {
        console.log('[DeleteDeletionRectangle] Deleting rect', id, 'from view', rectView);
        deleteDeletionRectangle(id, rectView);
      };
      
      const restoreFunc = (r: DeletionRectangle, view: ViewMode) => {
        console.log('[DeleteDeletionRectangle] Restoring rect', r.id, 'to view', view);
        restoreDeletionRectangle(r, view);
      };

      const command = new DeleteDeletionRectangleCommand(deleteFunc, restoreFunc, rect, rectView as ViewMode);
      history.executeCommand(command);
    },
    [deleteDeletionRectangle, restoreDeletionRectangle, history, elementCollections]
  );
}
  ...
  return useCallback(
    (
      ...
      const addFunc = (img: ImageType) => {
        const id = addImage(src, x, y, width, height, page, view, supabaseMetadata);
        img.id = id;
        return id;
      };
...
export function useHandleDeleteImageWithUndo(
  ...
  return useCallback(
    (id: string, view: ViewMode) => {
      ...
      const imageView = elementCollections.originalImages.some((img: any) => img.id === id)
        ? "original"
      ...
      const command = new DeleteImageCommand(deleteFunc, restoreFunc, image, imageView as ViewMode);
      history.executeCommand(command);
    ...
    [deleteImage, restoreImage, history, elementCollections, selectedElementId, clearSelectionState]
  );
}

// Handler for updating an image with undo support
export function useHandleUpdateImageWithUndo(
  updateImage: any,
  history: any
) {
  return useCallback(
...
export function useHandleMultiDeleteWithUndo(
  ...
  return useCallback(() => {
    ...
    console.log('[MultiDelete] Starting multi-delete for', selectedElements.length, 'elements');
    ...
    selectedElements.forEach((el: any) => {
      if (el.type === 'textbox') {
        const tb = [...elementCollections.originalTextBoxes, ...elementCollections.translatedTextBoxes, ...elementCollections.finalLayoutTextboxes]
          .find((t: any) => t.id === el.id);
        if (tb) {
          elementsToDelete.textBoxes.push(tb);
          // Determine view
          const view = elementCollections.originalTextBoxes.some((t: any) => t.id === el.id) ? "original" as ViewMode :
                       elementCollections.translatedTextBoxes.some((t: any) => t.id === el.id) ? "translated" as ViewMode : 
                       "final-layout" as ViewMode;
          elementViews.textBoxes.set(tb.id, view);
        }
      } else if (el.type === 'shape') {
        const shape = [...elementCollections.originalShapes, ...elementCollections.translatedShapes, ...elementCollections.finalLayoutShapes]
          .find((s: any) => s.id === el.id);
        if (shape) {
          elementsToDelete.shapes.push(shape);
          // Determine view
          const view = elementCollections.originalShapes.some((s: any) => s.id === el.id) ? "original" as ViewMode :
                       elementCollections.translatedShapes.some((s: any) => s.id === el.id) ? "translated" as ViewMode : 
                       "final-layout" as ViewMode;
          elementViews.shapes.set(shape.id, view);
        }
      } else if (el.type === 'image') {
        const img = [...elementCollections.originalImages, ...elementCollections.translatedImages, ...elementCollections.finalLayoutImages]
          .find((i: any) => i.id === el.id);
        if (img) {
          elementsToDelete.images.push(img);
          // Determine view
          const view = elementCollections.originalImages.some((i: any) => i.id === el.id) ? "original" as ViewMode :
                       elementCollections.translatedImages.some((i: any) => i.id === el.id) ? "translated" as ViewMode : 
                       "final-layout" as ViewMode;
    ...
    deleteTextBox, deleteShape, deleteImage,
    restoreTextBox, restoreShape, restoreImage,
    history, elementCollections, multiSelection, clearSelectionState
  ]);
}