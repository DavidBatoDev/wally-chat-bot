...
export class GroupCommand implements Command {
  ...
  constructor(commands: Command[] = [], description?: string) {
    ...
    this.description = description || `Group operation (${commands.length} actions)`;
  }
  ...
  execute() {
    console.log(`[GroupCommand] Executing ${this.commands.length} commands atomically`);
    // Execute all commands at once (appears atomic to user)
    this.commands.forEach(cmd => cmd.execute());
  }

  undo() {
    console.log(`[GroupCommand] Undoing ${this.commands.length} commands atomically`);
...
export function useHistory() {
  ...
  const startGroup = useCallback((description?: string) => {
    if (currentGroupRef.current) {
      console.warn('[History] Group already in progress - ending previous group');
      endGroup();
    }
  ...
  const endGroup = useCallback(() => {
    if (currentGroupRef.current && !currentGroupRef.current.isEmpty()) {
      ...
      console.log('[History] Ending group with', group.getCommands().length, 'commands');
  ...
  const undo = useCallback(() => {
    ...
    if (command) {
      ...
      if (command instanceof GroupCommand) {
        console.log('[History] Undoing group with', command.getCommands().length, 'commands');
      }
      ...
      console.log('[History] After undo - undo stack:', undoStackRef.current.length);
      console.log('[History] After undo - redo stack:', redoStackRef.current.length);
      return true;
    }
  ...
  const redo = useCallback(() => {
    ...
    if (command) {
      ...
      if (command instanceof GroupCommand) {
        console.log('[History] Redoing group with', command.getCommands().length, 'commands');
      }
      ...
      console.log('[History] After redo - undo stack:', undoStackRef.current.length);
      console.log('[History] After redo - redo stack:', redoStackRef.current.length);
      return true;
    }
  ...
  const canUndo = useCallback(() => {
    return undoStackRef.current.length > 0 || (currentGroupRef.current && !currentGroupRef.current.isEmpty());
  }, []);
